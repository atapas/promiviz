...
console.log(
	`Initial delays: red - ${redDelay}, green - ${greenDelay}, blue - ${blueDelay}`
);
...
console.log(
	`Initial rejections: red - ${rejectRed}, green - ${rejectGreen}, blue - ${rejectBlue}`
);
...
const selectDelay = () => {
	...
	console.log(
		`Delays: red - ${redDelay}, green - ${greenDelay}, blue - ${blueDelay}`
	);
	selectValue();
};
...
const rejectColor = () => {
	...
	console.log(
		`Rejections: red - ${rejectRed}, green - ${rejectGreen}, blue - ${rejectBlue}`
	);
	selectValue();
};
...
const setValues = (color, value, duration) => {
	...
	document
		...
		.parentElement.setAttribute("class", value);
...
const log = (msg, isError) => {
	...
	isError
		? logListElem.setAttribute("class", "error")
		: logListElem.removeAttribute("class");
...
const handleAll = async () => {
	try {
		log(
			`🕛 Started handling all the colors using Promise.all([red, green, blue])`
		);
		...
		log(
			`✔️ Finished handling all the colors using Promise.all([red, green, blue])`
		);
	} catch (err) {
...
const handleAny = async () => {
	try {
		log(
			`🕛 Started handling any of the colors using Promise.any([red, green, blue])`
		);
		...
		setValues(color, color, "3s");
		log(
			`✔️Finished handling any of the colors using Promise.any([red, green, blue])`
		);
	} catch (err) {
...
const handleRace = async () => {
	try {
		log(
			`🕛 Started racing of the colors using Promise.race([red, green, blue])`
		);
		...
		setValues(color, color, "3s");
		log(
			`✔️ Finished racing of the colors using Promise.race([red, green, blue])`
		);
	} catch (err) {
		log(`❌ Rejected the color ${err}. Rejecting All`, true);
	}
...
const handleAllSettled = async () => {
	log(
		`🕛 Started settling of the colors using Promise.allSettled([red, green, blue])`
	);
	...
	log(
		`✔️ Finished settling of the colors using Promise.allSettled([red, green, blue])`
	);
};
...
const handleResolve = async () => {
	try {
		log(
			`🕛 Resolving all colors individually with Promise.resolve(color => red, green, blue)`
		);
		const red = new Promise((resolve, reject) => {
			setTimeout(() => {
				rejectRed ? reject("red") : resolve("red");
		...
		log(
			`✔️ Finished resolving ${redResolved} using Promise.resolve(${redResolved})`
		);
		setValues(redResolved, redResolved, "3s");

		const greenResolved = await Promise.resolve(green);
		log(
			`✔️ Finished resolving ${greenResolved} using Promise.resolve(${greenResolved})`
		);
		setValues(greenResolved, greenResolved, "3s");

		const blueResolved = await Promise.resolve(blue);
		log(
			`✔️ Finished resolving ${blueResolved} using Promise.resolve(${blueResolved})`
		);
